# Ed25519 自作 学習 TODO

RFC 8032 の Ed25519 を Go でゼロから実装する。`math/big` と `crypto/sha512` のみ使用。
各段は「これが出れば成功」を満たしたらチェックを入れる。

進める順番: **field → curve → sign**（下の層が固まってから次へ）。
運用: 親タスク 1 つ ≒ 1 PR。子を終えるたびに `[x]` を入れてコミット。着手中の親に WIP を付ける。
（親 = 到達する「状態（できること）」の一文。子 = その手順。テスト緑は確認方法であって状態ではない）

> ★ 当面のゴール: **`TestAgainstStdEd25519`** を通すこと。
> （標準 crypto/ed25519 と keygen/sign/verify がバイト一致 = Ed25519 コア完成）
> ph/ctx/batch は応用でゴールではない。

---

## 0. 環境構築
- [x] `go version` 確認（go1.24.7）
- [x] `ed25519-study/` 作成・`go mod init`
- [x] 雛形作成（のち main.go / main_test.go の 1 ファイルずつに統合）
- [x] `go run .` と `go test ./...` が通る最小状態
- [ ] RFC 8032 本文を一度ざっと読む（§5.1 が Ed25519） … まず全体像の把握が目的

## 実装

- [x] GF(p) で加減乗・逆元が計算できる
  - [x] テスト先書き（feAdd/Sub/Mul/Inv、未実装は skip）
  - [x] feAdd 実装
  - [x] feSub 実装
  - [x] feMul 実装
  - [x] feInv 実装

- [ ] 曲線上の点を加算・スカラー倍できる（WIP）
  - [x] d = -121665/121666 を定義
  - [ ] 基点 B
  - [x] onCurve（点が曲線式を満たすか） … まず恒等元 (0,1) で確認。B が入ったら強化
  - [ ] add（点の加算）
  - [ ] scalarMul（スカラー倍）

- [ ] seed から鍵を作り、署名・検証できる
  - [ ] encode/decode（点の 32byte 変換。平方根の求め方は要調査）
  - [ ] generateKey（SHA-512→クランプ→A=a·B）
  - [ ] sign（R||S の 64byte）
  - [ ] verify（8·S·B == 8·R + 8·k·A）
  - [ ] TestAgainstStdEd25519 の Skip を外して緑（コア完成）

## 4. 仕上げ・検証の強化
- [ ] RFC 8032 のテストベクタ（既知の seed/msg/署名）と完全一致を確認
- [ ] 標準ライブラリ `crypto/ed25519` の出力と突き合わせる
- [ ] `main.go` を鍵生成→署名→検証のデモに差し替える
- [ ] （調査）タイミング攻撃など「学習実装ゆえ本番非推奨」な点を README にメモ

## 発展（学習・任意）: 自作実装を自分で攻撃してみる
- [ ] 署名や公開鍵から秘密鍵を復元できないか試す（復元できたら「作れていない」証拠）
- [ ] よくある実装ミスを突く: nonce r の使い回し、クランプ漏れ、検証式の抜けなど
- [ ] 目的はあくまで学習（自分の実装の検証）。他者の鍵・システムには使わない

---

### まだ決めていない/要調査（タスク化しておく）
- [ ] 座標系: アフィンのまま進めるか、途中で拡張座標へ移すか判断する
- [ ] `feSub` などで負値を正に畳む書き方を統一する（ヘルパを作るか）
- [ ] テストの置き場所: パッケージ内 `_test.go` のままで十分か確認
